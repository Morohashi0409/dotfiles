...
name: x-impact
description: >
  Analyze X (Twitter) posts for viral potential and optimize them using the actual recommendation algorithm.
...
- If `TodoWrite` is available, use it for progress tracking.
- If `TodoWrite` is not available, provide short textual progress updates instead.
- Keep the scoring report structure the same in both cases.
...
If `TodoWrite` is available, use it to show analysis progress with these tasks:
...
If `TodoWrite` is unavailable, skip the tool call and move through the same four steps with concise textual updates.
